...
const project=process.cwd(),target=path.join(project,'dist');
...
await fs.writeFile(path.join(target,'image-compression.json'),JSON.stringify({method:'lossless PNG re-encoding; unchanged decoded RGBA; authoring sources untouched',scope:'all PNG files in dist, not the first-load budget',before,after,saved:before-after,files:results},null,2));
