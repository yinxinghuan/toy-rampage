...
const base=process.env.QA_BASE||'http://127.0.0.1:5191/',pass=process.env.QA_PASS||'first';
...
async function run(width,height,lang='zh',reduced=false,full=false){
 ...
 if(process.env.QA_DEBUG)await p.evaluate(()=>{window.pointerTrace=[];for(const type of ['pointerdown','pointermove','pointerup','pointercancel'])window.addEventListener(type,e=>{window.pointerTrace.push({type,x:e.clientX,y:e.clientY,id:e.pointerId,target:e.target.className});window.pointerTrace=window.pointerTrace.slice(-20);});});
 ...
 const shot=async name=>p.screenshot({path:out+pass+'-v022-platform-layout-'+width+'-'+lang+'-'+name+'.png',fullPage:true});
 ...
 async function gesture(from,to,{cancel=false,inspect=false}={}){
  ...
  await stable(before);assert.equal(await p.locator('#coins').textContent(),coins);
  if(inspect){await shot('held-footprint');if(process.env.QA_DEBUG)console.log(JSON.stringify({width,lang,expected:{sx,sy,tx,ty},actual:await p.evaluate(()=>({trace:window.pointerTrace,ghost:document.querySelector('.tw__drag').getAttribute('style')}))}));}
 ...
 await shot('expand-before');assert.equal(await btn('main').isVisible(),false);await tap(btn('expand'));await shot('expand-after');await gesture(source('tray'),cell(0,3));assert.equal(await stage(),'ready');await shot('ready');
 ...
 {
  const mirror=new Workshop();mirror.place('tray',0,0);for(let i=0;i<100;i++)mirror.tick(.05);mirror.place('tray',0,0);mirror.expand();mirror.place('tray',0,3);mirror.place('shop:2',1,0);mirror.place('shop:1',0,3);
  assert.equal(await btn('refresh').isVisible(),false);

  for(let wave=0;wave<(full?5:2);wave++){
   ...
   await tap(btn('main'));mirror.startWave();
   assert.equal(await btn('main').isVisible(),false);
   if(wave===0)assert.equal(await btn('speed').isVisible(),false);
   if(wave===1){assert.match(await btn('speed').textContent(),/1×/);await tap(btn('speed'));assert.match(await btn('speed').textContent(),/2×/);await shot('speed-2x');await tap(btn('speed'));assert.match(await btn('speed').textContent(),/1×/);}
   await step(1500);await shot('wave-'+(wave+1));
   ...
   if(wave===0){
    assert.equal(await p.locator('.tw').getAttribute('data-lesson'),'refresh');assert.equal(await btn('refresh').isVisible(),true);
    await step(900);await shot('refresh-free');assert.equal(await p.locator('.tw__guide').isVisible(),true);
    const money=mirror.coins,beforeUnits=await p.locator('.tw__cells').innerHTML();
    await tap(btn('pause'));await step(200);await tap(btn('resume'));assert.equal(await p.locator('.tw').getAttribute('data-lesson'),'refresh');
    if(lang==='en'){await btn('refresh').focus();await p.keyboard.press('Enter');await step(60);}else await tap(btn('refresh'));
    mirror.refresh();await step(450);
    assert.equal(await p.locator('#coins').textContent(),String(money));assert.equal(await p.locator('.tw__cells').innerHTML(),beforeUnits);
    assert.equal(await p.locator('.tw').getAttribute('data-lesson'),'place');await shot('refresh-new-stock');
    const move=mirror.shopMove();assert.ok(move);await gesture(source(move.source),cell(move.c,move.r),{cancel:true});
    assert.equal(await p.locator('.tw').getAttribute('data-lesson'),'place');assert.equal(await p.locator('#coins').textContent(),String(money));
    await gesture(source(move.source),cell(move.c,move.r),{inspect:true});mirror.place(move.source,move.c,move.r);
    assert.equal(await p.locator('.tw').getAttribute('data-lesson'),'done');assert.equal(await p.locator('#coins').textContent(),String(money-3));
    await shot('refresh-lesson-done');await tap(btn('refresh'));mirror.refresh();await step(450);
    assert.equal(await p.locator('#coins').textContent(),String(money-5));await shot('refresh-paid');
   }
  }
  if(full){await shot('result');assert.equal(await stage(),'win');}
 }
...
try{if(process.env.QA_CASE==='en')await run(320,568,'en',true);else{await run(390,844,'zh',false,true);await run(320,568);await run(320,568,'en',true);await run(440,600);}await fs.writeFile(out+pass+'-v022-results.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results));}finally{await browser.close();}
